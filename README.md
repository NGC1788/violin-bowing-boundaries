...
불확실성을 고려한 활–현 경계 예측 연구를 위한 저장소입니다. 현재 버전은 Ubuntu GPU 환경 설치, 환경·합성 신호 점검, 공개 데이터 다운로드·자동 재개, 압축 목록 검사와 제한된 추출·CSV 앞부분 확인을 제공합니다. **학습 모델과 실제 연구 결과는 아직 구현 또는 검증되지 않았습니다.**
