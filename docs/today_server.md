...
> **2026-09-17:** 첫 데이터 준비는 `TODAY DATA PREPARATION: PASS`로 끝났다. 다음 단계는 [전체 시행 감사](trial_audit.md)다.
...
1. Part 1 버전 `17749111`의 `2024-03-25_TypeA_sample1.7z` **한 개만** 받는다. 이전 공식 메타데이터가 `data/manifests/`에 있으면 요청·해결 버전, 출처 URL, 파일명·크기·MD5를 고정값과 대조해 재사용한다. `Using validated saved metadata`가 나오면 메타데이터 API를 다시 호출하지 않는다. 유효한 기록이 없을 때만 API에서 조회한다. 기존 `.part`의 실제 길이에서 이어받는다. 일시적인 연결 끊김에는 최대 8번 추가 재시도한다. 재시도 간격은 10초부터 늘어나 최대 60초다. 무한 재시도는 하지 않는다.
...
| 표시 / 상황 | 다음 행동 |
|---|---|
| `Metadata: HTTP 504` 반복 | 현재 코드를 받아 재실행하면 검증 가능한 기존 manifest를 우선 사용한다. 기존 파일을 지우거나 `catalog`를 반복 호출할 필요가 없다. 캐시가 없고 API도 계속 실패하면 오류를 보존한다. |
...
메타데이터 재사용은 이 명령의 고정 버전에만 적용한다. 일반 `catalog`와 `download` 명령은 계속 공식 API를 조회한다. `metadata_used.json`과 완료 요약은 사용한 메타데이터 및 캐시 출처를 기록한다. 캐시를 사용하더라도 파일 전체 MD5와 정확한 이어받기 범위는 검증하며, 파일 전송 서비스 자체의 장애까지 해결되는 것은 아니다.
