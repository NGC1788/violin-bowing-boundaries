...
case "$TASK_COMMAND" in
  ...
  *) echo 'Usage: bash scripts/run.sh {setup-tools|doctor|smoke|catalog|download|archive-audit|prepare-first|trial-audit|grid-summary|regime-map|collection|jupyter|python} [arguments]' ;;
esac
